...
/**
 * GET    /.netlify/functions/admin-careers   — list submissions
 * PATCH  /.netlify/functions/admin-careers   — mark read/unread
 * DELETE /.netlify/functions/admin-careers   — delete
 */
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'PATCH') {
    ...
    if (readAll) {
      const { error } = await getSupabase().from('career_submissions').update({ read_at: new Date().toISOString() }).is('read_at', null);
      if (error) return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: error.message }) };
      return { statusCode: 200, headers: CORS, body: JSON.stringify({ ok: true }) };
    }
    if (!id) return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'id or readAll required' }) };
    const { error } = await getSupabase().from('career_submissions').update({ read_at: read ? new Date().toISOString() : null }).eq('id', id);
    if (error) return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: error.message }) };
    return { statusCode: 200, headers: CORS, body: JSON.stringify({ ok: true }) };
  }
  ...
  if (event.httpMethod === 'DELETE') {
    ...
    const { error } = await getSupabase().from('career_submissions').delete().eq('id', id);
    if (error) return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: error.message }) };
    return { statusCode: 200, headers: CORS, body: JSON.stringify({ ok: true }) };
  }
  ...
  try {
    ...
    let query = getSupabase()
      ...
      .select('id, name, email, phone, role, license_number, experience, why_bliss, portfolio_url, availability, read_at, created_at')
